...
AbstractModel.use = function(method, plugin) {
  if (typeof method === 'string') {
    if (!method.match(/(find|findAll|count|remove|save)/)) {
...
AbstractModel.run = function(ev, args, callback, stopOnResult, model) {
  var model_listeners = this.listeners('before ' + ev).slice(0);
  var listeners = (model ? model.listeners('before ' + ev).slice(0) : []);
  var self = (model || this);
  var Model = this;

  // Call each "before EVENT" handler in series.
  (function next(err, result) {
    if (err || (stopOnResult && result)) {
      done(err, result);
    }
    else if (model_listeners.length) {
      var applied = (model ? [model].concat(args) : args.slice(0));
      applied.push(next);
      model_listeners.shift().apply(Model, applied);
    }
    else if (listeners.length) {
      listeners.shift().apply(model, args.concat([next]));
    }
    else {
      done(null, result);
    }
  ...
  function done(err, result) {
    ...
    if (model) {
      Model.emit('after ' + ev, model, (args.length && result));
...
AbstractModel.find = function(query, callback) {
  ...
  return this.run('find', [query], callback, true);
};
...
AbstractModel.findAll = function(query, callback) {
  ...
  this.once('before findAll', function(query, next) {
    next(null, []);
  });

  return this.run('findAll', [query], callback, true);
};
...
AbstractModel.count = function(query, callback) {
  ...
  this.once('before count', function(query, next) {
    next(null, 0);
  });

  return this.run('count', [query], callback, true);
};
