 ...
 * @return {Resource.Query}
 */
Resource.Query.prototype.withRelated = function(relations) {
...
Resource.trigger = function(event, args, callback) {
  ...
  function done (err, result) {
    ...
    if (resource) {
      Resource.emit.apply(Resource, [event, result].concat(instanceArgs));
...
Resource.get = function(query, callback) {
  ...
  if (arguments.length === 0) {
    return new this.Query(this, this.get);
  }
