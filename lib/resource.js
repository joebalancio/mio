 ...
 * @returns {Resource}
 * @fires before find one
 * @fires after find one
 */
Resource.findOne = function(query, callback) {
  ...
  /**
   * Run after {@link module:mio.Resource.findOne} at the beginning of its
   * callback.
   *
 ...
 * @returns {Resource}
 * @fires before find many
 * @fires after find many
 */
Resource.find = function(query, callback) {
  ...
  /**
   * Runs at the beginning of {@link module:mio.Resource.find} callback.
   *
...
Resource.count = function(query, callback) {
  ...
  /**
   * Run after {@link module:mio.Resource.count} at the beginning of its
   * callback.
   *
   * @event after count
   * @type {Function}
 ...
 * @returns {Resource}
 * @fires before update many
 * @fires after update many
 */
Resource.update = function (query, changes, callback) {
  ...
  /**
   * Run after {@link module:mio.Resource.update} at the beginning of its
   * callback.
   *
 ...
 * @returns {Resource}
 * @fires before destroy many
 * @fires after destroy many
 */
...
Resource.destroy = function(query, callback) {
  ...
  /**
   * Runs before {@link module:mio.Resource.destroy} callback.
   *
   * Asynchronous. Listeners run in series. If an error is passed as the first
   * argument to `next`, subsequent listeners are not executed and the `next`
   * arguments are passed to the callback for
   * {@link module:mio.Resource.destroy}.
   *
  ...
  /**
   * Run after {@link module:mio.Resource.destroy} at the beginning of its
   * callback.
   *
  ...
  return this.run('destroy many', [query], function(err) {
    ...
    /**
     * Callback for {@link module:mio.Resource.destroy}.
     *
...
Resource.run = function(ev, args, callback, opts) {
  ...
  var listeners = (resource && resource.listeners['before ' + ev]) || [];
  var self = (resource || this);
  var instance_args = (resource ? [resource].concat(args):args).concat([next]);
  var resource_args = args.concat([next]);
  var Resource = this;
  ...
  function next (err, result) {
    if (err || (opts.stopOnResult && result)) {
      ...
      resource_listeners.shift().apply(Resource, instance_args);
    }
    else if (listeners.length) {
      listeners.shift().apply(opts.resource, resource_args);
    }
    else {
...
Resource.prototype.save = function(callback) {
  ...
  /**
   * Runs before {@link module:mio.Resource#save} callback.
   *
   * Asynchronous. Listeners run in series. If an error is passed as the first
   * argument to `next`, subsequent listeners are not executed and the `next`
   * arguments are passed to the callback for
  ...
  /**
   * Run after {@link module:mio.Resource#save} at the beginning of its
   * callback.
   *
   * @event after create
   * @type {Function}
 ...
 * persistence plugins.
 *
 * @param {destroyInstanceCallback} callback
 * @returns {Resource}
 * @fires before destroy
 * @fires after destroy
 */
Resource.prototype.destroy = function(callback) {
  ...
  this.constructor.run('destroy', [], function(err) {
    ...
    /**
     * Callback for {@link module:mio.Resource.prototype.destroy}.
     *
